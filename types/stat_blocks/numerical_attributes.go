...
func ParseNumericalAttributesData(dict map[string]interface{}) (parse.Parseable, error) {
	...
	missingKey := errors.ValidateKeyExistance(dict, []string{"ArmorClass", "HitPoints", "Speed", "ReactionCount", "Strength", "Dexterity", "Constitution", "Intelligence", "Wisdom", "Charisma"})
	if missingKey != nil {
	...
	return NumericalAttributes{
		...
		ReactionCount: int(dict["ReactionCount"].(float64)),
